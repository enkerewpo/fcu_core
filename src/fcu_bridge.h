...
bool get_send_lock(void) { return send_lock; }

bool get_receive_lock(void) { return receive_lock; }

void set_send_lock(bool lock) { send_lock = lock; }

void set_receive_lock(bool lock) { receive_lock = lock; }
...
inline uint16_t rbGetCount(const RingBuffer *pRingBuff) {
  return (pRingBuff->wp - pRingBuff->rp + pRingBuff->length) %
         pRingBuff->length;
}
...
inline int8_t rbIsFull(const RingBuffer *pRingBuff) {
  return (pRingBuff->rp - pRingBuff->wp + pRingBuff->length - 1) %
             pRingBuff->length ==
         0;
}
