...
static const char *DRONE_IP = "192.168.0.100"; // ip
static const char *USB_PORT = "/dev/ttyACM0";  // usb虚拟串口文件描述符
...
void mav_send_arm(void) {
  ...
  mavlink_msg_set_mode_encode(mavlink_system.sysid, mavlink_system.compid,
                              &msg_setmode, &setmode);
...
void mav_send_takeoff(void) {
  ...
  mavlink_msg_command_long_encode(mavlink_system.sysid, mavlink_system.compid,
...
void mav_send_target(
    float target_pos_x, float target_pos_y, float target_pos_z, //单位：m
  ...
  mavlink_msg_set_position_target_local_ned_encode(
      ...
      &msg_position_target_local_ned, &set_position_target_local_ned);
  mavlink_send_msg(mav_chan, &msg_position_target_local_ned);
}

void mav_send_actuator_control(float control1, float control2, float control3,
                               float control4, float control5, float control6,
                               float control7, float control8) {
  ...
  mavlink_msg_set_actuator_control_target_encode(
      mavlink_system.sysid, mavlink_system.compid,
      &msg_set_actuator_control_target, &set_actuator_control_target);
...
void parse_data(void) {
  ...
  if (n) {
    ...
    for (int i = 0; i < n; i++) {
      if (mavlink_parse_char(chan, rbPop(&mav_buf_receive), &msg_received,
                             &status)) {
        // printf("Received \n");
        ...
        case MAVLINK_MSG_ID_HEARTBEAT:
          printf("001 Received heartbeat time: %fs, voltage:%fV, current:%fA, "
                 ...
                 (double)batt.voltages[1] / 1000,
                 (double)batt.current_battery / 100, position.hdg & 0xFF,
                 position.hdg >> 12);
        ...
        case MAVLINK_MSG_ID_GLOBAL_VISION_POSITION_ESTIMATE:
          mavlink_msg_global_vision_position_estimate_decode(&msg_received,
                                                             &pose);
          odom_pub.header.frame_id = "map";
          odom_pub.header.stamp = ros::Time::now();
          float quaternion_odom[4];
          mavlink_euler_to_quaternion(pose.roll, pose.pitch, pose.yaw,
                                      quaternion_odom);
        ...
        case MAVLINK_MSG_ID_SET_POSITION_TARGET_LOCAL_NED: //仅机载电脑接收该数据包
          mavlink_msg_set_position_target_local_ned_decode(
              &msg_received, &set_position_target);
          if (set_position_target.coordinate_frame ==
              MAV_FRAME_MISSION) { //设置自主飞行的goal
                                   //point,把前右上坐标转换为前左上
        ...
        case MAVLINK_MSG_ID_ATTITUDE_QUATERNION:
          mavlink_msg_attitude_quaternion_decode(&msg_received,
                                                 &attitude_quaternion);
          ...
          // printf("q1,q2,q3,q4,g1,g2,g3: %f %f %f %f %f %f
...
void odomHandler(const nav_msgs::Odometry::ConstPtr &odom) {
  if (time_odom <= time_start || mav_chan == MAVLINK_COMM_0) { // USB传输无需降频
    ...
    if (ros::Time::now().toSec() - time_odom <
        0.1) { //用网络通信，如果odom频率过高进行降频，降至10hz以下
      return;
    }
    time_odom = ros::Time::now().toSec();
  }

  Eigen::Vector3f position_map((float)odom->pose.pose.position.x,
  ...
  mavlink_msg_local_position_ned_cov_encode(
      mavlink_system.sysid, mavlink_system.compid, &msg_local_position_ned_cov,
...
void motionHandler(const geometry_msgs::PoseStamped::ConstPtr &odom) {
  ...
  Eigen::Vector3f position_map((float)odom->pose.position.x,
                               (float)odom->pose.position.y,
                               (float)odom->pose.position.z);
  float quaternion_odom[4] = {
      (float)odom->pose.orientation.w, (float)odom->pose.orientation.x,
      (float)odom->pose.orientation.y, (float)odom->pose.orientation.z};
...
void missionHandler(const geometry_msgs::InertiaStamped::ConstPtr &mission) {
  ...
  mav_send_target(px, py, pz, vx, vy, vz, ax, ay, az, yaw, yaw_rate);
}
...
int main(int argc, char **argv) {
  ...
  odom = nh.subscribe<nav_msgs::Odometry>("/vins_estimator/odometry_001", 100,
                                          odomHandler,
                                          ros::TransportHints().tcpNoDelay());
  cmd = nh.subscribe<std_msgs::Int16>("/fcu_bridge/command", 100, cmdHandler,
                                      ros::TransportHints().tcpNoDelay());
  mission = nh.subscribe<geometry_msgs::InertiaStamped>(
      "/fcu_bridge/mission_001", 100, missionHandler,
      ros::TransportHints().tcpNoDelay());
  ...
  motion = nh.subscribe<geometry_msgs::PoseStamped>(
      "/motion_001", 100, motionHandler, ros::TransportHints().tcpNoDelay());
  ...
  if (mav_chan == MAVLINK_COMM_0) {
    ...
    get_drone =
        connect(socket_cli, (struct sockaddr *)&drone_addr, sizeof(drone_addr));
  ...
  int tcp_delay = 1;
  setsockopt(socket_cli, IPPROTO_TCP, TCP_NODELAY, (void *)&tcp_delay,
             sizeof(tcp_delay));
